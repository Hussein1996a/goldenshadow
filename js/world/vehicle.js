...
const VehicleManager = {
  ...
  update(dt, playerPos) {
    ...
    this.checkPedestrianHits(dt);
  },

  /* ---------- car-to-car impacts ---------- */
  // Arcade-realistic rigid-body-ish response: bodies are pushed apart in
  // proportion to mass (heavier vehicles shove lighter ones more), impact
  // speed bleeds off as forward speed and a lateral shove, and hard hits
  // deal damage, spark, and (if the player is involved) jolt the driver.
  resolveVehicleCollisions(dt) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (dist < 0.001) { dx = 0.05; dz = 0; dist = 0.05; }
        ...
        // positive = the two bodies are closing along the contact normal
        const closing = (vax - vbx) * nx + (vaz - vbz) * nz;
        if (closing <= 1.2) continue;

        const sevA = closing * (massB / totalMass);
        const sevB = closing * (massA / totalMass);
        a.speed -= Math.sign(a.speed || 1) * clamp(sevA * 0.6, 0, Math.abs(a.speed) + 6);
        b.speed -= Math.sign(b.speed || -1) * clamp(sevB * 0.6, 0, Math.abs(b.speed) + 6);

        // shove sideways along the impact normal, expressed in each car's
        // own local left/right axis so it reads as a believable spin-out
        const laX = Math.cos(a.heading), laZ = -Math.sin(a.heading);
        const lbX = Math.cos(b.heading), lbZ = -Math.sin(b.heading);
        a.lateralSlip -= (nx * laX + nz * laZ) * closing * 0.45;
        b.lateralSlip += (nx * lbX + nz * lbZ) * closing * 0.45;

        const dmg = clamp(closing * 2.4, 0, 55);
        a.health -= dmg * (massB / totalMass) * 1.3;
        b.health -= dmg * (massA / totalMass) * 1.3;

        if (nowT - a.lastCrashT > 0.3 || nowT - b.lastCrashT > 0.3) {
          a.lastCrashT = b.lastCrashT = nowT;
          const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2;
          if (World && World.effects) World.effects.crashSpark(mx, mz);
          AudioMgr.thud();
          if (World.player && (World.player.inVehicle === a || World.player.inVehicle === b)) {
            World.player.damage(clamp(closing * 0.8, 0, 22), 'crash');
          }
        }
        if (a.health <= 0 && a.alive) this.destroy(a);
        if (b.health <= 0 && b.alive) this.destroy(b);
      }
    }
  },

  /* ---------- running down pedestrians ---------- */
  checkPedestrianHits(dt) {
    for (const v of this.list) {
      if (!v.alive) continue;
      ...
      for (const n of World.npcs) {
        ...
        const d = Math.sqrt(dx * dx + dz * dz);
        if (d > v.radius + n.radius + 0.25) continue;
        n.runOver(v);
      }
    }
  },
